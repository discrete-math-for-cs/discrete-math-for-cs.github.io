...
environment in CSE 020 that supports your learning
...
Please reach out (minnes@ucsd.edu) if you need support with extenuating circumstances affecting CSE 020.

\vfill

\newpage
\section*{Welcome to CSE 020: Discrete Math for CS in Fall 2026!}
...
CSE 020 team: One instructor + two TAs and three tutors + all of you
...
\item Gradescope
\item Piazza
\item Course calendar for HW due dates and Q\&A, tests, etc.
\end{enumerate}
\end{multicols}
...
{\bf Pro-tip}: you can use MATH 109 to replace CSE 020 for prerequisites and other requirements.

\vfill

\section*{Themes and applications for CSE 020}
\begin{itemize}
...
\begin{list}
   {\itemsep2pt}
   \item \#FinAid Assignment on Canvas \url{https://canvas.ucsd.edu/courses/77744/assignments/1192300} 
   \item Read the syllabus on Canvas
   \item Schedule your Test 1 and Test 2 times at PrairieTest (http://us.prairietest.com)
   \item Review quiz based on class material (due Monday of Week 2)
   \item Create a homework group, possibly by using the Piazza (https://piazza.com/ ) find-a-teammate tool
   \item Homework assignment 1 (due Tuesday of Week 2)
\end{list}

\newpage

\section*{Week 0 Friday: Modeling applications}
...
\newpage
\section*{Week 1 Monday: Defining sets}
...
all natural numbers & $\mathbb{N}$ & The (set of all) natural numbers (nonnegative integers). {\it We use the convention that $0$ is a natural number.} \\
...
\vfill 
\section*{Week 1 Wednesday: Defining functions}
...
\section*{Week 1 Friday: Sets, functions, and algorithms}
...
$\textbf{ div } : \mathbb{Z} \times \mathbb{Z}^+ \to \mathbb{Z}$ given by $\textbf{ div } ( ~(n,d)~)$ 
is the quotient when $n$ is the dividend and $d$ is the divisor.

$\textbf{ mod } : \mathbb{Z} \times \mathbb{Z}^+ \to \mathbb{Z}$ given by $\textbf{ mod } ( ~(n,d)~)$ 
